...
# 백업 옵션
COMPRESSION_LEVEL=9      # 압축 레벨 (1-9, 9가 최대 압축)
PARALLEL_JOBS=4          # 병렬 처리 작업 수
...
perform_database_backup() {
    ...
    log_message "INFO" "Database size: $(get_database_size $db_name)"
    
    # pg_dump 실행 (custom format for flexibility)
    if pg_dump -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" \
        ...
        local file_size=$(du -h "$backup_file" | cut -f1)
        log_message "SUCCESS" "Database backup completed: $db_name (Size: $file_size)"
        
        # 백업 검증
        verify_backup "$backup_file" "database"
        
        # 메타데이터 저장
        save_backup_metadata "$backup_file" "$backup_type" "database" "$db_name"
        
        return 0
    else
        log_message "ERROR" "Database backup failed: $db_name"
        return 1
    fi
}
...
perform_physical_backup() {
    ...
    if pg_basebackup -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" \
        -D "$backup_path" \
        -Ft -z -Xs -P \
        --checkpoint=fast 2>>"${BACKUP_LOG_DIR}/backup_${TODAY}.log"; then
        
        local dir_size=$(du -sh "$backup_path" | cut -f1)
        log_message "SUCCESS" "Physical backup completed. Size: $dir_size"
        
        # 메타데이터 저장
        save_backup_metadata "$backup_path" "$backup_type" "physical"
...
save_backup_metadata() {
    ...
    local checksum=$(sha256sum "$backup_path" 2>/dev/null | cut -d' ' -f1)
