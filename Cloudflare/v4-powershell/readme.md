아래는 완전한 Windows DDNS 클라이언트(PowerShell) + Task Scheduler 자동 설치/삭제까지 포함한 단일 스크립트임. (IPv4 A / IPv6 AAAA 지원, 캐시/로그/자동 생성 옵션 포함)

Cloudflare는 Zone 조회(GET /zones) → DNS 레코드 조회(GET /zones/{zone_id}/dns_records) → **DNS 레코드 업데이트(PATCH /zones/{zone_id}/dns_records/{dns_record_id})**로 처리함.

📄출처: [Cloudflare, List Zones, 2026, https://developers.cloudflare.com/api/resources/zones/methods/list/], [Cloudflare, List DNS Records, 2026, https://developers.cloudflare.com/api/resources/dns/subresources/records/methods/list/], [Cloudflare, Update an existing DNS record (PATCH), 2026, https://developers.cloudflare.com/api/python/resources/dns/subresources/records/methods/edit/]

⸻

1) 완전한 Windows DDNS 클라이언트: cf-ddns.ps1
...
Task Scheduler 자동 설치/등록은 PowerShell의 Register-ScheduledTask 기반으로 구현함.

📄출처: [Microsoft, Register-ScheduledTask, 2026, https://learn.microsoft.com/en-us/powershell/module/scheduledtasks/register-scheduledtask?view=windowsserver2025-ps]
...
# 1) 작업 폴더(원하는 곳)로 저장
# 예: C:\script\cf-ddns.ps1

# 2) 설정 생성
powershell -ExecutionPolicy Bypass -File C:\script\cf-ddns.ps1 -InitConfig

# 3) config.json 수정
notepad "$env:ProgramData\cf-ddns\config.json"

# 4) 1회 실행 테스트
powershell -ExecutionPolicy Bypass -File C:\script\cf-ddns.ps1 -Run

# 5) 스케줄 설치(5분)
powershell -ExecutionPolicy Bypass -File C:\script\cf-ddns.ps1 -InstallTask -IntervalMinutes 5
...
A안) PowerShell 스크립트 → EXE 변환(PS2EXE)

가장 빠름. (실행은 PowerShell 런타임/환경 영향 받을 수 있음)

# 관리자 PowerShell 권장
Install-Module ps2exe -Scope CurrentUser
Import-Module ps2exe

Invoke-ps2exe .\cf-ddns.ps1 .\cf-ddns.exe -noConsole

PS2EXE는 .ps1을 .exe로 패키징하는 PowerShell 모듈임.

📄출처: [PowerShell Gallery, QuickSoft / New-Ps2exe, 2026, https://www.powershellgallery.com/packages/QuickSoft/1.2.4/Content/Public%5CNew-Ps2exe.ps1], [ITPro Today, How To Use PS2EXE To Convert PowerShell Scripts Into EXE Files, 2024, https://www.itprotoday.com/powershell/how-to-use-ps2exe-to-convert-powershell-scripts-into-exe-files]

⸻

B안) “진짜” 단일 EXE(권장): C#(.NET)로 구현 후 Single-file Publish

운영 안정성/배포성 최고. PublishSingleFile + SelfContained로 단일 파일 배포 가능.

📄출처: [Microsoft, 단일 파일 배포(Overview), 2025, https://learn.microsoft.com/ko-kr/dotnet/core/deploying/single-file/overview]

예시 명령(콘솔 앱 기준):

dotnet new console -n CfDdns
cd .\CfDdns

# (여기에 Cloudflare DDNS 로직 구현 후)

dotnet publish -c Release -r win-x64 --self-contained true /p:PublishSingleFile=true
# 결과: .\bin\Release\<TFM>\win-x64\publish\CfDdns.exe

단일 파일 배포는 OS/아키텍처별로 publish 필요.

📄출처: [Microsoft, 단일 파일 배포(Overview), 2025, https://learn.microsoft.com/en-us/dotnet/core/deploying/single-file/overview]
