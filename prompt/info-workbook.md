...
각 작업 단위는 하나 이상의 메인에이전트 또는 서브에이전트가 담당할 수 있으며, 담당 에이전트는 자신의 작업 범위에 대해서만 계획, 구현, 검증 및 문서를 작성한다.

검증은 항상 별개의 에이전트 인스턴스를 띄워서 아래 과정을 2회 반복한다.
- 서로 다른 관점에서 비판적으로 검토하는 서브에이전트 N개를 만들어 병렬로 기존 작업에 대해 비판하도록해. 
- 돌아온 비판이 타당하면 메인 세션에서 재검증하고 타당하면 수용하도록.
...
- 중요도가 높은 모든 작업에서 서브 에이전트 사용을 고려하십시오.
- 현재 체크리스트 항목 내에서 독립적인 연구, 검색, 검증 또는 분리된 구현 작업을 병렬로 실행하여 명확하게 도움이 될 수 있는 경우 서브 에이전트를 적극적으로 활용하십시오.
- 순차적인 단계, 중복되는 파일 편집 또는 밀접하게 연관된 리팩토링에는 서브 에이전트를 사용하지 마십시오.
- 서브 에이전트를 생성할 때는 메인 에이전트와 동일한 모델을 사용하십시오. 사용자가 명시적으로 다른 모델을 요청하지 않는 한 서브 에이전트 모델을 재정의하지 마십시오.
- 메인 에이전트는 계획, 통합, 최종 검증 및 사용자 커뮤니케이션을 담당합니다.
...
|------|------|----------------|
| orders | 현재 해야 할 일 | 항상 |
| plans | 다음 작업 기준 | 현재 단계만 |
| working | 진행 기록 | 현재 Stage Summary만 |
| report | 최종 결과 | 종료 후만 |
| feedback | 사람 리뷰 | 필요 시 |
| tech | 영구 지식 | 필요 시 |
| spec | 영구 설계 | 필요 시 |
| troubleshootings | 영구 교훈 | 필요 시 |

### 컨텍스트 원칙

AI는 이전 대화를 이어받지 않는다.

항상 다음 우선순위로 컨텍스트를 구성한다.

1. orders
2. 현재 단계 계획서
3. 직전 Stage Summary
4. 필요한 코드
5. 필요한 영구 문서(tech/spec/troubleshootings)

전체 working이나 report를 다시 읽는 것은 원칙적으로 금지한다.
