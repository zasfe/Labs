...
이 문서는 보고서 HTML 파일을 만들 때 따르는 디자인 시스템 정본이다. **HTML의 형식(골격, 색, 폰트, 컴포넌트, SVG, 조작 스크립트)만** 다루며, 무엇을 쓸지에 대한 콘텐츠 규칙(필수 포함 항목, 입력 문서, 퀴즈 주제 범위 등)은 프로젝트별 콘텐츠 가이드 문서에 따로 둔다.
